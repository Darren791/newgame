...
class Post(SharedMemoryModel):
    ...
    def display_post(self, player, show_replies=False):
        ...
        header = headers.header(self.db_board.name, player=player, width=player.screenwidth)

        # header = ("===[ " + postid + " ]").ljust(75, "=")

        # c1 = player.options.column_names_color
        c1 = "C"

        post_string = header
        post_string += f"|{c1}  PostID|n: {self.db_board.id}/{post_num} {'(Pinned)' if self.db_pinned else ''}\n"
        post_string += f"|{c1}    Date|n: {datestring}\n"
        post_string += f"|{c1}  Poster|n: {self.db_poster_name}\n"
        post_string += f"|{c1} Subject|n: {self.db_subject}\n"
        post_string += headers.subheader('', player=player, width=player.screenwidth)
        ...
        if show_replies:
            ...
            for r in replies:
                ...
                post_string += headers.subheader('', player=player, width=player.screenwidth)
                post_string += f"|{c1}Date   :|n {datestring}\n"
                post_string += f"|{c1}Poster :|n {r.db_poster_name}\n"
                post_string += "--------\n"
                post_string += r.db_text + "\n"

        post_string += headers.footer('', player=player, width=player.screenwidth)
...
class BoardDB(TypedObject):
    ...
    __settingclasspath__ = "boards.boards.DefaultBoard"
    __defaultclasspath__ = "boards.boards.DefaultBoard"
    __applabel__ = "boards"
