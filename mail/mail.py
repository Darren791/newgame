...
class CmdMail(default_cmds.MuxCommand):
    ...
    Switches:
      ...
      unread  - Marks the message as unread.
    Examples:
      @mail 2
    ...
    """
    key = "mail"
    switch_options = ("delete", "forward", "reply", "unread")
    lock = "cmd:all()"
    ...
    def send_mail(self, recipients, subject, message, caller):
        ...
        for recipient in recipients:
            if recipient.is_blocking(caller):
                continue
    ...
    def func(self):
        ...
        elif "unread" in self.switches or "unr" in self.switches:
            if self.lhs:
                try:
                    index = abs(int(self.lhs) - 1)
                except ValueError:
                    self.caller.msg("Mail ID must be an integer.")
                    return
                if index < 1:
                    self.caller.msg(f"That isn't a valid mail ID. Value must be a positive integer.")
                    return
                all_mail = self.get_all_mail()
                if not all_mail:
                    self.caller.msg("You dont't have any mail.")
                    return
                    
                mind_max = max(0, all_mail.count() - 1)
                try:
                    mind = max(0, mind_max, index)
                    message = all_mail[mind]
                except (ValueError, IndexError):
                    self.caller.msg("Mail ID out of range. Value must fall within the range of 1-{all_mail.count()}.")
                    return
                    
                message.tags.remove("new", category="mail")
                self.caller.msg(f"Mail ID {mind} marked as unread.")	
            else:
                self.caller.msg("Syntax: mail/unread <id>")
        else:
            # list messages 
            if messages := self.get_all_mail():
                self.caller.msg(header(f"Mail for {self.caller.key}(#{self.caller.id}):", self.caller, width=79))
            
                table = self.styled_table("ID", "From", "Subject", "Arrived", "Status", width=79)
                ...
                self.caller.msg(footer("", self.caller, 79))
            else:
